...
if(isset($_POST['registro'])){
    ...
    if ($stmt->fetchColumn() > 0) {
        header("Location: ../intro.php?status=error&message=El correo o el documento ya está registrado");
        exit();
    }
    ...
    if (!$row) {
        header("Location: ../ModEstudiante/perfil.php?status=orange&message=Usuario no encontrado.");
        exit();
    }
    ...
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] == 0) {
        ...
        if (in_array($fileType, $allowTypes)) {
            if (move_uploaded_file($_FILES['foto']['tmp_name'], $targetFilePath)) {
                ...
                header("Location: ../ModEstudiante/perfil.php?status=error&message=Error al subir la imagen.");
                exit();
            }
        } else {
            header("Location: ../ModEstudiante/perfil.php?status=error&message=Formato de imagen no permitido.");
            exit();
        }
    } else {
        $cnn = Conexion::getConexion();
...
if (isset($_POST['eliminar_foto'])) {
    ...
    if ($stmt->execute()) {
        header("Location: ../ModEstudiante/perfil.php?status=success&message=Foto eliminada correctamente.");
    } else {
        header("Location: ../ModEstudiante/perfil.php?status=error&message=No se pudo eliminar la foto.");
    }
    exit();
}
