...
if ($_SESSION['activo'] == 0) {
    header("Location: ./go.php?error=Debes activar tu cuenta desde el correo.");
    exit();
}
...
$asistencias = $uDao->listarAsistencias($fecha, $curso, $materia, $trimestre,);
...
<head>
    ...
    <script>
        ...
        function actualizarAsistencia(id) {
            ...
            fetch("controladores/controlador.asistencia.php", {
                method: "POST",
                body: formData
            })
            .then(res => res.json())
        ...
        function openList(element) {
            ...
            fetch("controladores/controlador.asistencia.php?idlistado=" + idListado)
                        ...
                        if (!empty($grado)) {
                            ...
                            if (!empty($lista)) {
                                foreach ($lista as $estudiante):
                                    // Se recomienda colocar el documento en un input oculto para enviarlo
                                ?>
                                    <tr>
                                        <td>
